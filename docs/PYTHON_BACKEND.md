...
```powershell
jakana debug examples/m.jk
```

The debugger operates on the Jakana AST, so pauses and line numbers refer to Jakana source rather than generated Python. A new session pauses at the first statement.
...
next             execute one statement, then pause
print EXPR       evaluate and inspect an expression
locals           show current variables
help             show commands
quit             stop debugging
```
...
```text
(jakana-debug) break 5
Breakpoint set at line 5
(jakana-debug) continue
Paused at line 5: print add(2, 3)
(jakana-debug) print add(2, 3)
5
(jakana-debug) continue
```
...
### Functions with `func` and `return`

Jakana functions use a simple declaration and an explicit `end` marker:
...
The parenthesized form `print(add(2, 3))` is also accepted, and `func add(...)` remains a compatibility alias. The simple Jakana form is preferred.

If `end` is missing, Jakana reports `Expected: end` and `Found: end of file`.
...
- `print expression` and `print(...)`
- functions with `name(parameters): ... end`
- execution through the installed `jakana` command
- source-level debugging with `jakana debug`
